...
*이슈 #317 — 비동기 버튼 toast 피드백 구현 및 전 브라우저 검증*
...
있을 때:
  노팀장 → "toast 추가, 디자인 가이드 확인:
            - BottomNav 높이 52px + safe area → mobileOffset 80px 설정 ✅
            - 모바일 뷰포트 확인 ✅"
  이팀장 → "가이드 대비 구현 검토, APPROVE"
  → 버그가 구현 단계에서 차단됨
```

라이브러리의 구체적인 동작 방식은 에이전트가 항상 정확히 파악하지 못할 수 있습니다. 하지만 "이 UI 요소는 BottomNav 위에 있어야 한다"는 규칙은 에이전트가 확인할 수 있습니다. **디자인 가이드는 에이전트의 판단 오류를 보완하는 가드레일입니다.**
...
[사람 개입 1] 시나리오 검수
    ↓
태요원: Playwright 테스트 코드 60개 작성
    ↓
    ├─ [버그 1] <Toaster /> 누락 → 노팀장 수정
    ├─ [버그 2] 알림 팝업 간섭 → 노팀장 수정
    └─ [버그 3-1차] BottomNav 가림 → 수정 (절차 위반: 태요원이 직접 처리)
    ↓
60/60 통과
    ↓
[사람 개입 2] 테스트 영상 검수 → "toast가 안 보인다"
    ↓
[버그 3-2차 발견] 모바일 뷰포트에서 mobileOffset 미적용
    ↓
노팀장: mobileOffset 추가, useAuth Firestore 타임아웃 추가
    ↓
태요원: 재검증 — boundingBox 측정으로 위치 수치 확인
    ↓
60/60 통과 (49초)
    ↓
PR 생성
